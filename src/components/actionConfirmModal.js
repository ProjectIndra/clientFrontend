...
const ActionConfirmModal = ({ visible, type="", onConfirm, onCancel, message, copyToken=false, command, isConfirmButtonVisible=true, isCancelButtonVisible=true, confirmButtonName, cancelButtonName }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-palette-surface p-6 rounded shadow text-center w-auto min-w-[320px] max-w-[90vw] md:max-w-[700px]">
                <p className="text-lg font-semibold text-palette-textPrimary mb-4">
                    {message || `Are you sure you want to ${type} this VM?`}
                </p>
                {copyToken && (
                    <div className="mt-2 bg-palette-surface border border-palette-border rounded-xl px-3 py-3 w-full flex gap-3 items-start text-left shadow-sm mb-4">
                        <code className="text-sm text-palette-textPrimary break-all whitespace-pre-wrap flex-1">
                            {bashScript}
                        </code>
                        <button
                            onClick={handleCopy}
                        ...
                        >
                            {copied ? (
                                ...
                                <div className="flex items-center gap-2">
                                    <CopyIcon className="h-5 w-5 text-palette-textSecondary" />
                ...
                <div className="flex justify-center gap-4">
                    ...
                    {isCancelButtonVisible && (
                        <button
                            onClick={onCancel}
                            className={`px-4 py-2 bg-palette-surfaceMuted text-palette-textSecondary rounded hover:${cancelButtonName === "Done" ? "bg-lime-300" : "bg-red-100"}`}
                        >
